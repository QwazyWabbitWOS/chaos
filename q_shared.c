...
void PerpendicularVector(vec3_t dst, const vec3_t src)
{
	...
	*/
	for (pos = 0, i = 0; i < 3; i++)
	{
		if (fabsf(src[i]) < minelem) /* MrG{DRGN} changed to float version */
		{
			pos = i;
			minelem = fabsf(src[i]); /* MrG{DRGN} changed to float version */
		}
	}
...
#if !defined (_WIN32) || !defined (_M_IX86) /* MrG{DRGN} */
...
/* MrG{DRGN} already defined in cirecrt_math.h
...
void Info_SetValueForKey(char* s, char* key, char* value)
{
	...
	int		c;
	size_t	maxsize = MAX_INFO_STRING;	 /* MrG{DRGN} changed to size_t */
