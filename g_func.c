...
void Move_Begin(edict_t* ent)
{
	...
	frames = floorf((ent->moveinfo.remaining_distance / ent->moveinfo.speed) / FRAMETIME); /* MrG{DRGN} use the float version of floor */
...
void plat_CalcAcceleratedMove(moveinfo_t* moveinfo)
{
	...
	if ((moveinfo->remaining_distance - accel_dist - decel_dist) < 0)
	{
		...
		moveinfo->move_speed = (-2 + sqrtf(4 - 4 * f * (-2 * moveinfo->remaining_distance))) / (2 * f); /* MrG{DRGN} use the float version of sqrt */
...
void plat_spawn_inside_trigger(edict_t* ent)
{
	...
	/* tmin[2] = ent->mins[2]; MrG{DRGN} reassigned below before use! */
...
void button_killed(edict_t* self, edict_t* inflicter, edict_t* attacker, int damage, vec3_t point)
{
...
void door_killed(edict_t* self, edict_t* inflicter, edict_t* attacker, int damage, vec3_t point)
{
	edict_t* ent;
...
void SP_func_water(edict_t* self)
{
	...
	self->classname = "func_door"; /* MrG{DRGN} is this right? It was just "func_door" when I got here. */
	self->classindex = FUNC_DOOR; /* MrG{DRGN} I guess we go with it */
	gi.linkentity(self);
}
...
void door_secret_die(edict_t* self, edict_t* inflicter, edict_t* attacker, int damage, vec3_t point)
{
	self->takedamage = DAMAGE_NO;
...
void SP_func_door_secret(edict_t* ent)
{
	...
	if (ent->spawnflags & SECRET_1ST_DOWN)
		width = fabsf(DotProduct(up, ent->size)); /* MrG{DRGN} use the float version of fabs */
	else
		width = fabsf(DotProduct(right, ent->size)); /* MrG{DRGN} use the float version of fabs */
	length = fabsf(DotProduct(forward, ent->size)); /* MrG{DRGN} use the float version of fabs */
	if (ent->spawnflags & SECRET_1ST_DOWN)
