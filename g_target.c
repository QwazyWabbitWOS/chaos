...
void target_explosion_explode(edict_t* self)
{
	...
	T_RadiusDamage(self, self->activator, (float)self->dmg, NULL, self->dmg + 40.0F, MOD_EXPLOSIVE);/* MrG{DRGN} type conversion */
...
void use_target_splash(edict_t* self, edict_t* other, edict_t* activator)
{
	...
	if (self->dmg)
		T_RadiusDamage(self, activator, (float)self->dmg, NULL, self->dmg + 40.0F, MOD_SPLASH); /* MrG{DRGN} type conversion */
}
...
void use_target_blaster(edict_t* self, edict_t* other, edict_t* activator)
{
	...
	fire_blaster(self, self->s.origin, self->movedir, self->dmg, (int)self->speed, effect, MOD_TARGET_BLASTER);	 /* MrG{DRGN} type conversion and EF_BLASTER should be effect! */
...
void target_earthquake_think(edict_t* self)
{
	...
	if (self->last_move_time < level.time)
	{
		...
		self->last_move_time = level.time + 0.5F; /* MrG{DRGN} type conversion */
	}
	...
	for (i = 1, e = g_edicts + i; i < globals.num_edicts; i++, e++)
	{
		...
		e->velocity[2] = self->speed * (100.0F / e->mass);/* MrG{DRGN} type conversion */
	}
