...
void fire_rail(edict_t* self, vec3_t start, vec3_t aimdir, int damage, int kick)
{
	...
	while (ignore && i < 1000)/* MrG{DRGN} railgun runaway trace fix*/
	{
		...
		{
			/* MrG{DRGN} Zoid added so rail goes through SOLID_BBOX entities (gibs, etc) */
			if ((tr.ent->svflags & SVF_MONSTER) || (tr.ent->client) ||
		...
		i++;/* MrG{DRGN} railgun runaway trace fix*/
	}
...
void fire_bfg(edict_t* self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius)
{
	...
	/* MrG{DRGN} these are changed before use below!
